...
HRESULT DirectX11Manager::Init(HINSTANCE hInstance, int cCmdShow)
{
	...
	hWnd = CreateWindow(wcex.lpszClassName, "InstancingGrass", WS_DLGFRAME,
...
DXGI_FORMAT DirectX11Manager::GetDxgiFormat(D3D10_REGISTER_COMPONENT_TYPE type, BYTE mask)
{
...
ID3D11InputLayout* DirectX11Manager::CreateInputLayoutReflection(const string& filename, const string& entrypath, vector<string> macros)
{
	ID3D11InputLayout* pVertexLayout;
	UINT	compileFlags = 0;
	ComPtr<ID3DBlob> blob;
	...
	mbstowcs(ws, filename.c_str(), 512);
	ComPtr<ID3DBlob> pErrorBlob = NULL;
	vector<D3D_SHADER_MACRO> shaderMacro;
	for (auto& m : macros)
	{
		D3D_SHADER_MACRO tmp = { m.c_str(),"1" };
		shaderMacro.push_back(tmp);
	}

	D3D_SHADER_MACRO null = { NULL,NULL };
	shaderMacro.push_back(null);

	HRESULT hr = D3DCompileFromFile(ws, shaderMacro.data(), D3D_COMPILE_STANDARD_FILE_INCLUDE, entrypath.c_str(), "vs_5_0", compileFlags, 0, &blob, &pErrorBlob);
	// エラーチェック.
	if (FAILED(hr))
	{
		// エラーメッセージを出力.
	...
	hr = D3DReflect(blob->GetBufferPointer(), blob->GetBufferSize(), IID_ID3D11ShaderReflection, &pReflector);
	if (FAILED(hr))
		return nullptr;
	...
	D3D11_INPUT_ELEMENT_DESC* layout = new D3D11_INPUT_ELEMENT_DESC[shaderdesc.InputParameters];
	for (size_t i = 0; i < shaderdesc.InputParameters; ++i)
	{
		D3D11_SIGNATURE_PARAMETER_DESC desc;
		auto cb = pReflector->GetInputParameterDesc((UINT)i, &desc);
		auto format = GetDxgiFormat(desc.ComponentType, desc.Mask);
	...
	hr = m_pDevice->CreateInputLayout(layout, shaderdesc.InputParameters, blob->GetBufferPointer(),
		blob->GetBufferSize(), &pVertexLayout);
	assert(SUCCEEDED(hr));
	delete[] layout;
...
ID3D11ShaderResourceView* DirectX11Manager::CreateTextureFromFile(const wchar_t* filename)
{
	...
	{
		TexMetadata meta;
		GetMetadataFromWICFile(filename, WIC_FLAGS_NONE, meta);

		std::unique_ptr<ScratchImage> image(new ScratchImage);
		HRESULT hr = LoadFromWICFile(filename, WIC_FLAGS_NONE, &meta, *image);
		if (FAILED(hr))
			return nullptr;
		hr = CreateShaderResourceView(m_pDevice.Get(), image->GetImages(), image->GetImageCount(), meta, &ShaderResView);
		if (FAILED(hr))
...
void DirectX11Manager::DrawBegin()
{
	...
	// 指定色で画面クリア
	float ClearColor[4] = { 0.3f, 0.8f, 1.0f, 1.0f }; //red,green,blue,alpha
